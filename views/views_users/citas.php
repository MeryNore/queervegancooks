<?php
    ...
    try{
        $select_stmt = $mysqli_connection->prepare('SELECT * FROM citas WHERE idUser = ? ORDER BY fecha_cita ASC'); 
        ...
        if($result -> num_rows > 0){
            # Inicializamos un array vacío para almacenar las citas
            $citas = [];
            # fetch_assoc() nos permite obtener los datos de las citas como un array asociativo (clave: valor), como fetch trae sólo el primer resultado, añadimos $fila para que mientras existan datos los traiga en una fila nueva
...
<body>
    ...
    <main>
        ...
        <div class="container-fluid">
            <h2 class="my-4 text-center"> GESTIÓN DE CITAS</h2>
            ...
            <div class="container d-flex flex-wrap justify-content-center">
                <form id="citas_form" name="citas_form" action="../../controllers/user_citas/c_citas.php" method="POST">
                    <ul class="p-0">
                        <li>
                            <label for="nombre">NOMBRE</label>
                            <input disabled type="text" value="<?php echo $user_data['nombre']; ?>">
                        </li>
                        <li>
                            <label for="email">EMAIL</label>
                            <input disabled type="email" value="<?php echo $user_data['email']; ?>">
                        </li>
                        <h4 class="p-2 bg-success-subtle rounded text-center">AGENDAR UNA CITA</h4>
                        <li>
                            <label for="fecha_cita">FECHA CITA</label>
                            <input type="date" id="fecha_cita" name="fecha_cita" title="Puedes solicitar cita desde mañana y hasta con un año de antelación" required>
                            <small class="input_error alert alert-danger p-1 m-0" role="alert"></small>
                        </li>
                        <li>
                            <label for="motivo_cita">MOTIVO</label>
                            <textarea name="motivo_cita" id="motivo_cita" rows="4" cols="40" placeholder="Escribe brevemente el motivo de tu cita" title="Intenta no introducir caracteres especiales"></textarea>
                            <small class="input_error alert alert-danger p-1 m-0" role="alert"></small>
                        </li>
                        <!-- BOTONES  -->
            ...
            <!-- GESTIÓN DE CITAS -->
            <div class="container-fluid">
                <h4 class="p-2 bg-success-subtle rounded text-center">CITAS AGENDADAS</h4>
                    ...
                    <table class="table table-light table-striped table-hover">
                        ...
                        <?php if(isset($citas)){
                            foreach ($citas as $citas){?>
                                <tr>
                                    <td><?php $fecha = date('d-m-Y', strtotime($citas['fecha_cita'])); echo $fecha; ?></td>
                                    <td><?php echo $citas['motivo_cita']; ?></td>
                                    <td class="text-end"><small class="liButton">
                                            <a href="./citas_editar.php?idCita=<?php echo $citas['idCita']; ?>"><input type="submit" name="editar_cita" value="EDITAR"></a>
                                            <a href="../../controllers/user_citas/c_eliminar_citas.php?borrar_cita=<?php echo $citas['idCita']; ?>"><input type="submit" name="borrar_cita" value="BORRAR"></a>
                                        </small>
                                    </td>
                                </tr>
                        </tbody>
